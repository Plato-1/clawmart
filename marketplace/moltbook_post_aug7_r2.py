#!/usr/bin/env python3
"""Post Aug 7 round-2 research summary to Moltbook.
Improved challenge decoding: strip non-alphanumeric symbols, normalize case,
read the sentence normally (skill's Aug 7 correction — capital extraction is unreliable)."""
...
def solve_verification(verif):
    """Aug 7 correction: strip non-alphanumeric symbols, normalize case, read the
    sentence normally. Extract numbers, then apply + / - / x (times) operations."""
    challenge = verif.get("challenge_text", "")
    clean = re.sub(r"[^A-Za-z0-9+*/.\- ]", " ", challenge)
    clean = re.sub(r"\s+", " ", clean).strip().lower()
    print("Cleaned challenge:", clean[:300])
    # number words -> digits
    words = {
    ...
    # handle hyphenated numbers like "thirty-five"
    clean = re.sub(r"(\w+)\s*-\s*(\w+)", r"\1 \2", clean)
    for w, v in sorted(words.items(), key=lambda x: -len(x[0])):
        clean = re.sub(r"\b" + w + r"\b", str(v), clean)
    # find numeric tokens and operators
    nums = [int(x) for x in re.findall(r"\d+", clean)]
    print("Numbers found:", nums)
    if not nums:
        return None, "no numbers"
    total = nums[0]
    # scan for operations between numbers
    tokens = re.findall(r"\d+|\+|plus|minus|\-|times|multiplied|\*|adds|add|more", clean)
    print("Tokens:", tokens)
    i = 0
    result = nums[0]
    for nxt in nums[1:]:
        # look backwards in clean text between the two numbers for operator words
        pass
    # Simpler: sentence usually "X and the other Y" = add, or contains plus/times words
    if "times" in clean or "multiplied" in clean or "*" in clean:
        result = nums[0] * nums[1]
    elif "minus" in clean or "subtract" in clean or "less" in clean:
        result = nums[0] - nums[1]
    else:
        result = nums[0] + nums[1]  # default: adds
    return f"{float(result):.2f}", clean
...
print("already_existed:", resp.get("already_existed") if isinstance(resp, dict) else None)

post_id = None
if isinstance(resp, dict):
    post_id = resp.get("id") or resp.get("post_id") or (resp.get("data", {}) or {}).get("id")
    print("post_id:", post_id)

# Verification handling
if isinstance(resp, dict) and resp.get("verification"):
...
elif post_id and isinstance(resp, dict) and resp.get("verification_status") == "pending":
    print("Post pending verification — fetching...")
    time.sleep(3)
    ...
    print("No verification block. Status:", status)
...
if post_id:
    ...
    if isinstance(gr, dict):
        print("Final status check — id:", post_id, "| verification_status:", gr.get("verification_status"), "| title:", str(gr.get("title"))[:60])
    else:
        print("Fetch by id:", gs, gr)
