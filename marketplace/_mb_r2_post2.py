#!/usr/bin/env python3
"""Post with DISTINCT title -> fresh verification block -> solve -> confirm."""
...
TITLE = "10 research findings on monetizing AI agents in 2026 — all shipped as ClawMart kits today"
...
if isinstance(resp, dict):
    ...
    if verif:
        ...
        print("no verification block; status pending?", vs)
        time.sleep(3)
        gs, gr = api_get(f"/posts/{pid}")
        if isinstance(gr, dict):
            p2 = gr.get("post", gr)
            verif2 = p2.get("verification")
            print("refetch verification:", json.dumps(verif2)[:300] if verif2 else "NONE", "| status:", p2.get("verification_status"))
