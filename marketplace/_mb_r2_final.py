#!/usr/bin/env python3
"""Final Moltbook post — fresh title, robust compound-number solver."""
...
TITLE = "AI agent monetization 2026: 10 research-backed moves, now live as ClawMart kits"
...
def text_to_numbers(s):
    """Convert all number words (incl. compounds like 'twenty-three') to digits."""
    # Handle hyphenated compounds first: twenty-three -> 23
    def compound(m):
        parts = m.group(0).replace("-", " ").split()
        if len(parts) == 2 and parts[0].lower() in TENS and parts[1].lower() in UNITS:
            return str(TENS[parts[0].lower()] + UNITS[parts[1].lower()])
        return m.group(0)
    s = re.sub(r"\b(?:twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety)-(?:one|two|three|four|five|six|seven|eight|nine)\b", compound, s, flags=re.I)
    # Then single words
    def single(m):
        v = word_num(m.group(0))
        return str(v) if v is not None else m.group(0)
    s = re.sub(r"\b(?:zero|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety|hundred)\b", single, s, flags=re.I)
    return s
...
def solve(verif):
    ...
    t = text_to_numbers(clean)
    print("NUMERIZED:", t[:300])
    nums = [float(x) for x in re.findall(r"\d+(?:\.\d+)?", t)]
    print("NUMS:", nums)
    if len(nums) < 2:
        return None, "need 2+ numbers"
    if "multipli" in clean or "times" in clean or "product" in clean or "x" in clean:
        r = nums[0] * nums[1]
    elif "divid" in clean or "quotient" in clean or "split" in clean:
        r = nums[0] / nums[1]
    elif "minus" in clean or "subtract" in clean or "slows" in clean or "less" in clean or "differ" in clean or "decreas" in clean or "reduce" in clean:
        r = nums[0] - nums[1]
    else:
        r = nums[0] + nums[1]
    return f"{r:.2f}", t
...
if isinstance(resp, dict):
    ...
    vs = post.get("verification_status") or resp.get("verification_status")
    print("verification_status:", vs)
    if verif:
        answer, expr = solve(verif)
        vcode = verif.get("verification_code")
        print("ANSWER:", answer)
        if vcode and answer:
            vs2, vr = api_post("/verify", {"verification_code": vcode, "answer": answer})
            print(f"VERIFY: {vs2} -> {json.dumps(vr)[:300]}")
            time.sleep(6)
            gs, gr = api_get(f"/posts/{pid}")
            if isinstance(gr, dict):
                p2 = gr.get("post", gr)
                print("FINAL:", pid, "| status:", p2.get("verification_status"), "| title:", str(p2.get("title"))[:60])
    else:
        print("no verification block in response")
